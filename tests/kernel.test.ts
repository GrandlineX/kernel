...
describe.each([
  ["KernelDefault",9900,false],
  ["KernelLight",9901,true]]
)(
  'V(%s,%i):',
  (instanceName, port ,sqLight) => {
    let kernel = testKernelUtil(instanceName,port);
    ...
    test('keystore', async () => {
      ...
      if (cc) {
        ...
        const wrapper=kdb.getEntityWrapper<GKey>("GKey")
      }
    });
    ...
    test('db function', async () => {
      ...
      await db?.setConfig("test","test")
      expect(await db?.configExist("test")).toBeTruthy();
